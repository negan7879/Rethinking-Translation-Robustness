...
aug_times = 15
train_mode_grid = 2
data_root = "/datb/notebook/CPS/homework/myIDEA/data/cifar-10-batches-py/hdf5"

trainDataset = cifarDataset(train_mode="train",grid=grid_size,data_root=data_root,canvas=canvas)
validDataset = cifarDataset(train_mode="val",grid=grid_size,data_root=data_root,canvas=canvas)
...
if aug_flag == False:
    output_dir = "./output/exp{}/{}_grid_{}_exponent_{}_lr_{}_canvas_{}".format(exp, model.__class__.__name__, grid_size, exponent,
                                                                      lr, canvas)
else:
    output_dir = "./output/exp{}/{}_grid_{}_exponent_{}_lr_{}_aug_{}_canvas_{}".format(exp, model.__class__.__name__,
                                                                                grid_size, exponent,
                                                                                lr, aug_times,canvas)
if os.path.exists(output_dir) is False:
    os.makedirs(output_dir)
with open(os.path.join(output_dir, "README"), "a") as fp:
    print("aug_times = {}".format(aug_times), file=fp)
